...
import { deleteUserById, getAllUsers, getUserById, saveNewUser, updateUser } from "../../services/typeorm/fetchDB/user"
...
userRouter
  ...
  .put(async (req: Request, res: Response) => {
    const result: User | void = await updateUser(req.body).catch((err) => console.log(err))
    if (!result) return res.status(500).json(new Error("Impossible to update the user"))
